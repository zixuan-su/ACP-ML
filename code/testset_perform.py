...
def test_dataset(path):
    fp = open(r'../performance/test.txt', 'a')  # 用于记录训练集性能指标
    ...
    model = joblib.load(r'../model/StackingClassifier_et.pkl')
   
    y_predict_class = model.predict(y_data)
    y_predict_probo = model.predict_proba(y_data)
    
    lst_1 = y_predict_class.tolist()
    lst_2 = y_predict_probo[:,1].tolist()
    ...
    #final.to_csv('../file/Votingsoft.csv', header=None, index=None)

    acc, SN, SP, mcc, auc_score = performance(y_lable, y_predict_class, lst_2)
    print('-----------------在测试集上的性能为-----------------------------')
    ...
    fp.write("特征名: " + str(path).split('/')[2] + ',' + " 分类器名: " + str(model).split('(')[0] + '\n')
...
if __name__ == '__main__':
    ...
    test_dataset(r'../data/RFECV_Test.csv')
