...
def performance(y, predicted_label, predicted_proba):
    ...
    return accuracy, sensitivity, specificity, precision, recall, f1, auc, MCC


def cross_validation_k(path):
    fp = open(r'../result/singlefeature.txt', 'a')  # 用于记录训练集性能指标
    # 导入特征文件
    # data_train = pd.read_csv(r'ap_train.csv')
    data_train = pd.read_csv(path)
    #data_train = data_train.dropna()
    # print(data_train)  # 输出了一下 是从第0行开始的，比如2400条序列，总行数就是0-2399，特征行显示但不计入总行数
    # data_train = preprocessing.scale(data_train)
    kfold = KFold(n_splits=10, shuffle=True)  # 初始化

    # shape()返回为列表类型表示数组的行列数，  iloc[   :  ,  : ]    前面的冒号就是取行数，后面的冒号是取列数，操作表格类型
    x_train = np.array(
        data_train.iloc[0:, :data_train.shape[1] - 1])  # 取所有行，取到倒数第二列，不要标签列

    y_train = data_train["class"]  # 训练集所有数据的标签
    # print(y_train)
    acc_sum = SN_sum = SP_sum = precision_sum = recall_sum = f1_sum = auc_sum = mcc_sum = 0  # 统计各种性能指标在每一折的表现之和，用于最后求平均
    tmp = 0
    i = 1
    ...
    lst_1 = pd.DataFrame(x_train)
    lst_2 = pd.DataFrame(y_train)
    ...
    final.to_csv('../data/smoteenn.csv', header=None, index=None)

    print(x_train.shape)

    #############################################################  change modle here  ######################################################
    emodel = SVC(probability=True)  # soft probability=True
    # rf = RandomForestClassifier()
    # gdbt = GradientBoostingClassifier()
    # nb = GaussianNB()
    # emodel = LGBMClassifier()
    # knn = KNeighborsClassifier()
    # lr = LogisticRegression()
    # adaboost = AdaBoostClassifier()
    #emodel = BaggingClassifier(base_estimator = SVC(), n_estimators=500)
    #emodel  = LGBMClassifier()
    #emodel = MLPClassifier()
    for train_index, test_index in kfold.split(x_train):
        ...
        #  y_train保存着所有数据的标签， x_train保存着所有的数据
        ...
        acc, SN, SP, precision, recall, f1, auc, mcc = performance(test_label, y_predict, y_predict_proba)
        print("第", i, "折各项评价指标为")
        ...
        print("Precision: %.3f" % precision, end='   ')
        print("Recall: %.3f" % recall, end='   ')
        print("F1: %.3f" % f1, end='   ')
        ...
        auc_sum += auc
        i += 1
    # joblib.dump(emodel, r'../finalmodel' + '//' + tmp +
    #             '.pkl')  # sklearn中的joblib模块进行模型保存与加载
    ...
    #print("sp: %.3f%%" % ((sp_sum / 10) * 100), end='   ')
    print("ACC: %.3f%%" % ((acc_sum / 10) * 100), end='   ')
    print("MCC: %.3f" % ((mcc_sum / 10)), end='   ')
    print("Precison: %.3f" % ((precision_sum / 10)), end='   ')
    print("Recall: %.3f" % ((recall_sum / 10)), end='   ')
    print("F1: %.3f" % ((f1_sum / 10)), end='   ')
    ...
    fp.write("    ACC: %.3f%%" % ((acc_sum / 10) * 100))
    fp.write("    MCC: %.3f" % ((mcc_sum / 10)))
    fp.write("    Precison: %.3f" % ((precision_sum / 10)))
    fp.write("    Recall: %.3f" % ((recall_sum / 10)))
    fp.write("    F1: %.3f" % ((f1_sum / 10)))
    fp.write("    AUC: %.3f\n" % ((auc_sum / 10)))
    fp.write('\n')
    fp.close()
    return 0
...
if __name__ == '__main__':
    ...
    cross_validation_k(r'../data/train/zuhe3.csv')
